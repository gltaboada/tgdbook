...
##       crecer 13.6638 14.486 15.8603 15.62375 17.4813
##  preasignado  3.8006  6.188  7.1354  7.25350  8.3659
##      max neval cld
##  18.7349    10  a 
##   9.3998    10   b
\end{verbatim}
...
## Unit: milliseconds
##        expr     min       lq      mean   median       uq
##  apply_mean 23.1764 25.29135 26.599060 26.50930 27.97680
##    colMeans  1.2329  1.33870  1.425380  1.43375  1.49715
##   apply_sum 14.9857 17.17095 23.269110 18.00785 18.80410
##     rowSums  1.2687  1.36790  2.016065  1.68185  2.38955
##       max neval cld
##   29.8401    20  a 
##    1.6456    20   b
##  125.8440    20  a 
...
Al cargar \texttt{tidyverse} es habitual que aparezcan avisos de \emph{conflictos}: algunas
funciones de estos paquetes tienen el mismo nombre que funciones de R base o
de otros paquetes ya cargados (por ejemplo, \texttt{dplyr::filter()} enmascara a
\texttt{stats::filter()}). Si esto ocurre, la función del paquete cargado en último
...
##     base_r 5.9441 11.92335 30.46873 12.86035 15.08850 364.818    20   a
##  dplyr_lag 4.4052  9.99200 28.43778 11.80500 14.35645 350.438    20   a
\end{verbatim}
...
Además, salvo que se indique lo contrario, se asume que las \(n\) observaciones de la muestra son \textbf{independientes entre sí} (y en muchos desarrollos teóricos, también idénticamente distribuidas, supuesto habitualmente conocido como i.i.d.). Cuando esto no se cumple ---por ejemplo con \textbf{series temporales} (observaciones consecutivas en el tiempo, ver \texttt{tidyverts}/\texttt{fable} en la Sección \ref{tidyverse}), datos \textbf{agrupados o longitudinales} (varias medidas repetidas sobre un mismo individuo) o datos \textbf{espaciales} (observaciones cercanas geográficamente más parecidas entre sí)--- la dependencia entre observaciones debe tenerse en cuenta explícitamente, tanto al ajustar el modelo como al evaluarlo (por ejemplo, particionar entrenamiento/test al azar dejaría de ser adecuado; ver Sección \ref{cv}). El tratamiento de datos dependientes requiere métodos específicos que quedan fuera del alcance de esta introducción.
...
\caption{\label{tab:gof-polyfit}Medidas de bondad de ajuste de los modelos polinómicos (muestra de entrenamiento).}\tabularnewline
\toprule\noalign{}
& \(MSE\) & \(R^2\) & \(R^2_{adj}\) \\
\midrule\noalign{}
\endfirsthead
...
\FunctionTok{matplot}\NormalTok{(grados, mse, }\AttributeTok{type =} \StringTok{"l"}\NormalTok{, }\AttributeTok{col =} \StringTok{"lightgray"}\NormalTok{, }\AttributeTok{lty =} \DecValTok{1}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{2}\NormalTok{),}
  \AttributeTok{xlab =} \StringTok{"Grado del polinomio (complejidad)"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Error cuadrático medio"}\NormalTok{)}
...
\FunctionTok{plot}\NormalTok{(}\FunctionTok{density}\NormalTok{(train[, }\StringTok{"medv"}\NormalTok{]))}
\FunctionTok{boxplot}\NormalTok{(train}\SpecialCharTok{$}\NormalTok{medv)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{lines}\NormalTok{(newdata}\SpecialCharTok{$}\NormalTok{lstat, }\FunctionTok{predict}\NormalTok{(fit}\FloatTok{.1}\NormalTok{se, }\AttributeTok{newdata =}\NormalTok{ newdata), }\AttributeTok{lty =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \DecValTok{2}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{3}\NormalTok{)}
...
\NormalTok{train }\OtherTok{\textless{}{-}}\NormalTok{ Boston[itrain, ]}
\NormalTok{test  }\OtherTok{\textless{}{-}}\NormalTok{ Boston[}\SpecialCharTok{{-}}\NormalTok{itrain, ]}
\CommentTok{\# Remuestra bootstrap (con reemplazamiento) de los índices de entrenamiento}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
...
También es habitual calcular medidas de error, por ejemplo con \texttt{caret::postResample()}:
...
\NormalTok{modelo\_ovo }\OtherTok{\textless{}{-}} \FunctionTok{svm}\NormalTok{(Species }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Petal.Length }\SpecialCharTok{+}\NormalTok{ Petal.Width, }\AttributeTok{data =}\NormalTok{ train3, }\AttributeTok{kernel =} \StringTok{"linear"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(p, lado, }\AttributeTok{type =} \StringTok{"b"}\NormalTok{, }\AttributeTok{ylim =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{xlab =} \StringTok{"Número de predictores (p)"}\NormalTok{,}
...
Para quien quiera profundizar en aprendizaje estadístico más allá de esta introducción, el libro completo de referencia dedica capítulos específicos a la regresión (selección de variables, regularización, regresión logística y multinomial), a la clasificación (más allá de KNN: árboles, SVM, evaluación específica con curvas ROC\ldots), a la regresión no paramétrica (splines, modelos aditivos, regresión local) y a la maldición de la dimensionalidad, contenidos que quedan fuera del alcance de esta asignatura pero pueden ser de interés para quien continúe por esa línea.
